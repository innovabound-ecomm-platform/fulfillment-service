...
import { PrismaClient, getFulfillmentPrisma } from "@innovabound-ecomm-platform/fulfillment-db";
...
async function runValidation() {
  ...
  try {
    checks.push({
      name: 'Import fulfillment-db module (PrismaClient)',
  ...
  // Check 2: getFulfillmentPrisma function exists
  try {
    checks.push({
      name: 'getFulfillmentPrisma function available',
      passed: typeof getFulfillmentPrisma === 'function',
    });
  } catch (error: any) {
    checks.push({
      name: 'getFulfillmentPrisma function available',
